...
从当前版本开始，每次更新条目都需要写明具体时间，格式为 `YYYY-MM-DD HH:MM:SS`。后续 git 提交备注统一使用中文。

## 2026-05-22

### 简化更新时间格式

更新时间：2026-05-22 10:22:00

- 删除更新时间里的时区后缀，让日志更简洁。
- 将时间格式统一为 `YYYY-MM-DD HH:MM:SS`。
- 后续更新日志继续保留具体到几点几分几秒的时间。

### 统一更新日志时间与中文提交备注

更新时间：2026-05-22 10:18:13

- 为所有已有更新条目补充具体到时分秒的时间。
- 明确后续每次更新日志都必须写明时间，不能只写日期。
- 明确后续 git 提交备注统一使用中文，避免继续使用英文提交说明。
- 保留已推送历史提交的原始记录，不直接强制改写远程提交历史。

### 回退为纯 Python 标准库基础版

提交时间：2026-05-22 10:15:31

- 按“不使用 numpy，只用最基础 Python 语法”的要求，将 `cdr_demo.py` 改为纯标准库实现。
- 移除 `numpy` 导入，改用 `random`、`math`、`statistics`、普通列表和 `for` 循环完成信号生成、插值、统计和 CDR 环路。
- 删除 `requirements.txt`，项目现在不需要安装任何第三方依赖。
- 更新 `README.md`，说明当前版本只需要系统自带 Python 3。
- 更新 `docs/CDR_DEMO_EXPLAINED.md`，用纯基础语法重新解释信号生成、固定采样、Alexander CDR 和输出文件。
- 保留 `UPDATE_LOG.md` 维护规则，并新增“更新时间”字段要求。

### 回退为 NumPy 基础版单文件 demo

提交时间：2026-05-22 10:07:11
